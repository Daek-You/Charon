...
public class VCam : MonoBehaviour
{
    ...
    void LateUpdate()
    {
        ...
        RaycastHit[] hits = Physics.RaycastAll(transform.position, direction, Mathf.Infinity, 1 << LayerMask.NameToLayer("Transparent"));

        for (int i = 0; i < hits.Length; i++)
        {
            Debug.Log($"ºÎµúÈù ¾Ö : {hits[i].transform.name}");
            TransparentObject[] obj = hits[i].transform.GetComponentsInChildren<TransparentObject>();

            for (int j = 0; j < obj.Length; j++)
            {
                Debug.Log($"ºÎµúÈù ¾ÖÀÇ ±×°Í: {obj[j]}");
